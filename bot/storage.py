...
def init_db():
    with _conn() as con:
        con.execute(
            "CREATE TABLE IF NOT EXISTS chats (chat_id INTEGER PRIMARY KEY, tournament_url TEXT, title TEXT)"
        )
        con.execute(
            "CREATE TABLE IF NOT EXISTS overrides (chat_id INTEGER, match_key TEXT, category TEXT, PRIMARY KEY(chat_id, match_key))"
        )

def set_tournament(chat_id: int, tournament_url: str, title: Optional[str] = None):
    with _conn() as con:
        con.execute(
            "REPLACE INTO chats (chat_id, tournament_url, title) VALUES (?, ?, ?)",
            (chat_id, tournament_url, title),
        )
...
def set_override(chat_id: int, match_key: str, category: str):
    with _conn() as con:
        con.execute(
            "REPLACE INTO overrides (chat_id, match_key, category) VALUES (?, ?, ?)",
            (chat_id, match_key, category),
        )
